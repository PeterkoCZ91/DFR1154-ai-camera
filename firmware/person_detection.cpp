...
bool PersonDetector::init() {
    ...
    #ifndef EI_CLASSIFIER_ALLOCATION_STATIC
    // Reserve PSRAM block for EI SDK tensor arena (ei_calloc → MALLOC_CAP_DEFAULT → PSRAM)
    ...
    initialized = true;
    Serial.printf("PersonDetector: Initialized (decode=%uB, input=%uB, arena=%s)\n",
                  PD_DECODE_BUF_SIZE, PD_INPUT_WIDTH * PD_INPUT_HEIGHT,
...
// JPEG → RGB565 (JPG_SCALE_8X) → grayscale → bilinear resize to 64×64
bool PersonDetector::prepareInput(const uint8_t* jpeg_data, size_t jpeg_len,
                                   int frame_w, int frame_h) {
    if (!initialized) return false;

    // Step 1: Decode JPEG to RGB565 at 1/8 scale → 200×150 (UXGA)
    // DC-only JPEG decode needs ~4x less heap than 1/4 scale, avoiding OOM during TLS uploads.
    ...
    bool converted = jpg2rgb565(jpeg_data, jpeg_len, grayscale_buffer, JPG_SCALE_8X);
    if (!converted) {
...
PersonDetectionResult PersonDetector::runInference() {
    ...
    float box_threshold = config.person_confidence_threshold - 0.15f;
    if (box_threshold < 0.35f) box_threshold = 0.35f;
    if (box_threshold > config.person_confidence_threshold) box_threshold = config.person_confidence_threshold;

    for (uint32_t i = 0; i < ei_result.bounding_boxes_count && detections < 16; i++) {
        float value = ei_result.bounding_boxes[i].value;
        if (value >= box_threshold) {
            track_dets[detections * 3 + 0] = (float)ei_result.bounding_boxes[i].x;
            track_dets[detections * 3 + 1] = (float)ei_result.bounding_boxes[i].y;
            track_dets[detections * 3 + 2] = value;
            detections++;
...
PersonDetectionResult PersonDetector::detectFromJpeg(const uint8_t* jpeg_data, size_t jpeg_len,
    ...
    if (!prepareInput(jpeg_data, jpeg_len, frame_w, frame_h)) {
        // Reset temporal counter on decode failure — otherwise a recovered
        // frame could pass the temporal filter on a single hit and fire a
        // false positive inherited from detections before the corruption.
        consecutive_detections = 0;
        last_result = {false, 0.0f, 0, 0};
        return last_result;
    }
