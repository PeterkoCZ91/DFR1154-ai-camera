...
// --- LAB MODE ---
// When true: HTTP auth is bypassed on mutable endpoints (/settings, /record,
// /sd-clear, /ir-control, /credentials, /update). Convenient for bench testing
// without juggling Basic Auth credentials, but UNSAFE on untrusted networks.
//
// Production default: false. Override at build time if you really need it on
// a trusted dev network:  pio run -DLAB_MODE=1
#ifndef LAB_MODE
#define LAB_MODE false
#endif
...
// 15s still drifted to ~44 KB in ~26 min. 30s halves the drift rate;
// bot command latency increases to ~15s avg which is acceptable.
#define TELEGRAM_POLL_INTERVAL_MS 30000

// --- TAMPER DETECTION ---
// Minimum lux reading required to consider camera_covered tamper.
// Camera dark + no motion is only flagged as tamper when the lux sensor
// sees ambient light (i.e. the lens should be seeing it too).
// In a genuinely dark room lux stays near zero → no false alert.
// Raise if your environment has strong ambient light even at night (street lamps etc.)
...
struct Config {
  ...
  // Motion detection / Motion → Telegram notifications
  bool motion_detection_enabled;    // Run firmware-side motion analysis (default true)
  ...
  bool person_telegram_photo;          // Send Telegram photo on CONFIDENT/UNCERTAIN detection (default true)
  ...
  int person_recheck_interval;         // Presence re-check interval in seconds (default 12); separate
                                       // from photo cooldown so stationary person is cleared faster
