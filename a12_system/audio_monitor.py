...
class AudioMonitor:
    def __init__(self, config: dict, callback=None):
        ...
        self.analysis_interval = max(0.05, float(audio_cfg.get("analysis_interval_seconds", 0.25)))
        ...
        # Rolling PCM buffer for AV clips (16kHz, 16-bit mono).
        buffer_seconds = max(5, int(audio_cfg.get("buffer_seconds", 20)))
    ...
    def _process_chunk(self, chunk: bytes) -> None:
        ...
        with self.lock:
            ...
            if len(self.audio_buffer) > self.buffer_size:
                del self.audio_buffer[: len(self.audio_buffer) - self.buffer_size]
