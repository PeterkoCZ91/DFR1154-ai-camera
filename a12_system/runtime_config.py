    ...
    def update_from_mqtt(self, topic: str, payload) -> bool:
        ...
        try:
            ...
            config_map = {
                ...
                "require_sensor": "require_sensor_for_recording",
                # Thresholds
                "motion_sensitivity": "motion.threshold",
                "yolo_confidence": "yolo.confidence_threshold",
                "notify_threshold": "event_scoring.notify_threshold",
                "local_record_threshold": "event_scoring.local_record_threshold",
                # Timing
                "periodic_yolo_interval": "periodic_yolo_interval",
                "pir_cooldown": "pir_recording.cooldown_seconds",
                "telegram_cooldown": "telegram_cooldown_seconds",
                "detection_cooldown": "detection_cooldown_seconds",
                # Media
                "telegram_media_mode": "telegram.media_mode",
                "telegram_enabled": "telegram.enabled",
                # Features
                "audio_detection": "audio_enabled",
                "face_recognition": "face_recognition.enabled",
            ...
            if key == "telegram_media_mode" and value not in ["mp4", "preview_mp4", "snapshot", "text", "none"]:
                logging.error(f"Invalid telegram_media_mode: {value}")
                return False

            if key in ("notify_threshold", "local_record_threshold") and not (0 <= int(value) <= 200):
                logging.error(f"Invalid score threshold: {value} (must be 0-200)")
                return False
