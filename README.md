...
[![Version](https://img.shields.io/badge/Version-3.12.42-blue)]()
...
> [!TIP]
> **New in v3.12.42** — Idle scan reduced 30 s → 5 s (temporal filter gets second frame while person is still visible). `PD_MIN_DETECTIONS` 2 → 1: single FOMO centroid is enough — temporal filter + ByteTrack reject false positives. `person_telegram_photo` flag added (independent of `motion_telegram_photo`). A12 MQTT `clean_session=True` fix. See [CHANGELOG](CHANGELOG.md).
>
> *v3.12.41* — TFLite Micro export + ESP-NN SIMD: inference ~3500 ms → ~300–500 ms (11× speedup). *v3.12.39* — UNCERTAIN detections routed via MQTT to A12 for YOLO verification. *v3.12.38* — AI fallback timeout 12 s → 5 s. *v3.12.36* — Three-state `PersonDecision` (NONE / UNCERTAIN / CONFIDENT).
>
