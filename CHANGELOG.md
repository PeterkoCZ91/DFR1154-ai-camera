...
### Fixed (A12 — the motion accuracy figure divided two different populations)

- **`record_motion_event` counted every YOLO invocation as a motion outcome.** It is called once per check, and most checks are not motion events at all — the periodic sweep and the PIR window both reach it with nothing moving — yet it always incremented a true or a false positive. The denominator counted only the runs where ESP32 or OpenCV motion actually fired. With the shipped `MOTION_THRESHOLD=0` and firmware motion off, that denominator is permanently zero, so the summary reported `accuracy: 0.0%` next to a false-positive count accumulated from checks that had no motion to be wrong about; in a mixed configuration the ratio could exceed 100%. A check with no motion is now not recorded as a motion event at all.
- The ratio is taken over the outcomes rather than the event count, so it stays defined if the two ever drift, and an invariant test pins that they do not. Swapping the denominator back is an equivalent mutant — after the fix the two are always equal — and is left as one rather than chased.
- The block is not persisted by `save()` and no consumer reads it, so nothing downstream changes. It was fixed rather than deleted because motion can be switched back on, and a silently wrong number is worse than no number.
...
- **`mqttInit()` now sets `setSocketTimeout(2)`.** `PubSubClient::connect()` blocks, and its default socket timeout is 15 seconds. `mqttLoop()` calls it from the Arduino main loop while holding `mqttLock()`, and `mqttReconnect()` retries every 10 s — so a camera that cannot reach or authenticate with the broker spends roughly a quarter of its life unreachable, and not only on MQTT: ICMP, the HTTP server and the detection stream all stop answering for the duration of each attempt. On the serial console every outage begins within a second of `MQTT: Failed, rc=-4` and lasts as long as the connect did. The two failure codes behave completely differently and that is what made this look environmental: `rc=5` (not authorised) returns immediately and costs nothing, while only `rc=-4` (timeout) blocks. Measured on one board, one position, 190 s of ping each time — 25.4 % loss with blackouts of 11, 11, 11 and 12 s before the change; 0.0 % and none after, with the same rejected credentials still driving the retry loop.
- **This retracts a hardware diagnosis.** Two days earlier the same board was declared to have a failing antenna, on the strength of a second board measuring clean in the same place — that board simply had working credentials and never entered the retry loop. The first one was moved to a different room, re-measured, and very nearly replaced. Before blaming hardware for periodic dropouts on these boards, disable MQTT and measure again.
- **`device_name` now defaults to `ESP32-Camera-<efuse-mac>` instead of a shared constant.** Two boards flashed from the same source claimed one mDNS name; the loser was renamed `-2` and `ESP32-Camera.local` then resolved to whichever device answered first. On 2026-09-12 that was a network printer, and the A12 companion streamed from it. Existing installs are unaffected — `device_name` lives in `/config.json` on LittleFS, which an OTA preserves, so the default only applies to a board with no config.
- Considered and deliberately **not** changed: the early return in `updateIRAutoMode()`. Read closely, `if (!irConfig.auto_mode) return;` is coherent rather than broken — `auto_mode` is the master switch for automatic control and `time_based` only selects the clock instead of the lux sensor. If the time window is ever wanted, the fix is configuration, not code.

### Documented

- How to read the camera's reboot log without inverting cause and effect: `reason=PANIC(4)` is the firmware crashing, while `reason=SW(3)` preceded by `reboot_cmd` is the companion's watchdog rebooting a camera whose stream just died. A crashing camera produces both, alternating. Count the panics, not the restarts.
- A checklist for putting a bench board into the production role. Pointing the companion at a board does not make it a production camera: firmware motion detection, person detection, Telegram and MQTT all belong off in the PIR-first architecture where the camera is only a frame source, and the IR LED belongs off because the LTR-308 is sealed behind the same plastic as the lens. Each of these was missed in one swap and each caused a separate incident — 45 restarts in a day, stall spikes of 105-141 an hour, and an IR LED running inside a closed box. With the LED on, that sealed sensor read 4.2 lux; with it off, 0.2. The LED was lighting its own sensor and still could not clear the 13 lux threshold.
- Enabling MQTT over `/settings` does nothing until a reboot: `mqttLoop()` returns early unless `mqtt_initialized` is set, and that only happens in `mqttInit()` at boot. The setting persists and `/settings` echoes it back, which is what makes it convincing — 190 s of silence on the broker, then the topic appears within seconds of a restart.

## [Unreleased] - 2026-09-14

### Fixed (A12 — darkness and a stopped sensor no longer look the same)

- **A dark hallway was being treated as a wedged exposure loop.** `classify_frame_health()` decided on `std` alone, so a genuinely dark scene — the hallway with the light off reads `brightness 5, std 0.1` — produced the same verdict as a sensor that had stopped reading out, and the recovery ladder spent its exposure-rewrite budget on a camera that was working correctly. Three strikes now pass without escalation where they used to open an episode.
- **The discriminator is frame-to-frame identity, not a brightness threshold.** Silicon always has read noise, so a live sensor never returns the same frame twice; a hung one repeats it byte for byte. Measured that evening on this camera at the 160x120 the heartbeat already computes — hung sensor: max|d| 0.00; the closest live case (dark hallway, AGC off): max|d| 1.00. The separation is categorical, so the test is equality rather than a fitted threshold, and `np.array_equal` compares shape as well, so a resolution change mid-episode reads as "not identical" instead of raising.
- **A proven hang now takes precedence over the exposure ladder** and goes straight to a bounded reboot ladder of its own (3 strikes ~= 90 s at the ~30 s heartbeat, 3 reboots, 120 s cooldown), because rewriting AEC/AGC registers cannot restart a readout that has stopped. The exposure ladder's give-up message no longer speculates about a blocked lens when the sensor is demonstrably still producing new frames.
- **Give-up latches are now per ladder.** They were sharing one `gaveup` key, so the exposure ladder giving up early in an episode would have swallowed the frozen-sensor ladder's power-cycle alert hours later — the one message that asks for someone to physically unplug the camera. Found by reading the live state file after deployment; the tests could not catch it because each starts from a fresh state.
- `test_stream_freeze_reboot.py` had been modelling darkness as 120 byte-identical frames, which is the definition of the failure it was supposed to exclude. It now feeds frames uniform to within one level but never twice the same, which is what a live sensor in the dark actually produces.

## [Unreleased] - 2026-09-13

### Changed (A12 — face recognition rebuilt around what the camera can actually see)

- **"No face was resolvable" and "a face was resolvable and matched nobody" are now different answers.** Only the second says anything about the person at the door, and the first is the common case: of 300 stored person frames only 22 % contain a detectable face at all, and only 6.7 % carry one large enough to embed. The detector already distinguished them internally, but carried the result as bare strings that four separate places re-hardcoded — and a fifth did not check at all, so the notification caption appended the sentinel verbatim and Telegram read *"Person detected (Video) (No face)"*. A `FaceOutcome` enum now carries the distinction, with one mapping for the labels already written to the events database. The daily summary reports strangers and unreadable frames separately, which is what says whether alerting on "unknown" was ever justified.
- **The check runs only inside the PIR window, and only on the YOLO person box.** A face measures a median 99 px when somebody presents themselves to the camera and a median 35 px in ordinary passage, so outside that window the check costs full price and answers almost nothing. The person box was already being computed and discarded; cropping to it with margin for the head is free. This bounds an always-on cost to a few frames per occurrence, which is what the original disable was about.
- **One verdict per occurrence instead of one per frame.** The rule is deliberately asymmetric, because recognition is used as a suppressor and never as a stranger detector: a resident verdict needs repeated sightings agreeing on one name, since a false accept there hides a real stranger, while a single unmatched face is enough to report one. Frames that resolved nothing never outvote anything.
- **The backend is OpenCV's own YuNet detector and SFace recogniser.** `dlib`, `face_recognition`, `onnxruntime` and `insightface` are all absent from the image — the lazy import could never have succeeded there — and dlib publishes no wheel, which is why it was never installed. Both OpenCV APIs run on `cv2.dnn`, the same engine the YOLO path already uses and under the same single-thread pinning, so enabling them costs two model files in the data directory and no new dependency. One check measures 64.5 ms against dlib's 251.9 ms.
- **Galleries are tagged with the backend that built them, and a mismatch is refused.** dlib and SFace embeddings are both 128-dimensional but live in different spaces, so no shape check can tell them apart, and comparing across them would produce confident nonsense. Since recognition only ever suppresses an alert, that failure mode means muting a real stranger.
- Measured while enrolling: adding 31 samples captured at the door lifted recall from 5/9 to 9/9 above 0.5 — and lifted an *unenrolled* person from 0.196 to 0.620, past the enrolled person's worst 0.531. More data made the matcher more sensitive and less discriminating. Suppression wants recall and authorisation wants precision, so they want different galleries.

### Fixed (A12 — the stall detection window, reverted)

- **Narrowing the window to 2 s / 3 s was wrong and is back at 18 s / 20 s.** The ordering invariant it shipped alongside is correct and stays: the socket read timeout must be strictly shorter than the freeze heuristic, or a transport break gets recorded as an image-quality event. But the floor is the camera's own per-frame send timeout of 15 s — below that, every hiccup the firmware would have recovered from by itself becomes a teardown, and a teardown costs a median 14 s to reconnect. In the hour after the change the companion tore the stream down 49 times against about one an hour before it.
